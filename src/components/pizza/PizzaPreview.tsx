...
function useSvgIds() {
  ...
  const p = (name: string) => `${raw}-${name}`
  return {
    emptyGrad: p('emptyGrad'),
    crustGrad: p('crustGrad'),
    doughGrad: p('doughGrad'),
    doughNoise: p('doughNoise'),
    crustEdge: p('crustEdge'),
    sauceGrad: p('sauceGrad'),
    sauceEdgeShadow: p('sauceEdgeShadow'),
    sauceNoise: p('sauceNoise'),
    pizzaShadow: p('pizzaShadow'),
    crustBump: p('crustBump'),
    pizzaClip: p('pizzaClip'),
    crustClip: p('crustClip'),
  }
}
    ...
    return (
      <div className={`pizza-preview pizza-preview--empty${compact ? ' pizza-preview--compact' : ''}`}>
        <svg viewBox="0 0 300 300" className="pizza-preview__svg">
          <defs>
            <radialGradient id={ids.emptyGrad} cx="40%" cy="38%">
  ...
  return (
    <div className={`pizza-preview${compact ? ' pizza-preview--compact' : ''}`}>
      <svg viewBox="0 0 300 300" className="pizza-preview__svg">
        <defs>
          ...
          {sauceColor && (
            <>
              ...
              <radialGradient id={ids.sauceEdgeShadow} cx="50%" cy="50%" r="50%">
                <stop offset="65%" stopColor="transparent" />
                <stop offset="100%" stopColor="rgba(0,0,0,0.38)" />
              </radialGradient>
              <filter id={ids.sauceNoise} x="0%" y="0%" width="100%" height="100%" colorInterpolationFilters="sRGB">
          ...
          <filter id={ids.pizzaShadow} x="-15%" y="-15%" width="130%" height="130%">
            <feDropShadow dx="0" dy="8" stdDeviation="14" floodColor="rgba(0,0,0,0.6)" />
          </filter>
          <filter id={ids.crustBump} x="0%" y="0%" width="100%" height="100%" colorInterpolationFilters="sRGB">
          ...
          <clipPath id={ids.pizzaClip}>
            <circle cx="150" cy="150" r="116" />
          </clipPath>
          <clipPath id={ids.crustClip}>
            <circle cx="150" cy="150" r="138" />
          </clipPath>
        </defs>

        <circle cx="152" cy="156" r="135" fill="rgba(0,0,0,0.4)" />
        <circle cx="150" cy="150" r="138" fill={`url(#${ids.crustGrad})`} filter={`url(#${ids.pizzaShadow})`} />
        <circle
          cx="150"
          cy="150"
          r="138"
          fill={crustMid}
          opacity="0.25"
          filter={`url(#${ids.crustBump})`}
          clipPath={`url(#${ids.crustClip})`}
        />
        ...
        <circle cx="150" cy="150" r="138" fill={`url(#${ids.crustEdge})`} clipPath={`url(#${ids.crustClip})`} />
        <circle cx="150" cy="150" r="118" fill={`url(#${ids.doughGrad})`} />
        <circle cx="150" cy="150" r="118" fill={crustMid} opacity="0.15" filter={`url(#${ids.doughNoise})`} />
        <circle cx="150" cy="150" r="118" fill="none" stroke="rgba(0,0,0,0.35)" strokeWidth="8" />

        {sauceColor && (
          <>
            <circle cx="150" cy="150" r="112" fill={sauceColor} opacity="0.92" filter={`url(#${ids.sauceNoise})`} />
            <circle cx="150" cy="150" r="112" fill={`url(#${ids.sauceGrad})`} />
            ...
            <circle cx="150" cy="150" r="112" fill={`url(#${ids.sauceEdgeShadow})`} />
          </>
        )}

        <g clipPath={`url(#${ids.pizzaClip})`}>
